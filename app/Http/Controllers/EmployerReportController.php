...
class EmployerReportController extends Controller
{
    ...
    public function genrate_report(Request $request)
    {
        ...
        if($team_member_details!="")
        {
            ...
            ->select('tbl_post_jobs.job_title as job_title','tbl_post_jobs.ID as Job_id',
            'tbl_post_jobs.job_code as job_code','tbl_team_member.team_member_type','tbl_team_member.company_id','tbl_team_member.full_name',
            'tbl_job_post_assign.team_member_id','tbl_team_member_type.type_name','tbl_team_member_type.type_ID',
            'tbl_job_post_assign.job_assigned_date','tbl_team_member.email as user_email_id','tbl_team_member.company_id','tbl_job_post_assign.org_id')
            ...
            ->where('tbl_job_post_assign.org_id',Session::get('org_ID'))
            ->where('tbl_job_post_assign.owner_id',$value_team['ID'])
            ->select('tbl_post_jobs.ID as Job_id',
...
// exit;
        ...
        $seeker_details=tbl_team_member_type::leftjoin('tbl_team_member','tbl_team_member.team_member_type','=','tbl_team_member_type.type_ID')
                        ...
                        ->select('tbl_team_member.ID as team_member_id','tbl_job_seekers.ID as seeker_id',
                        'tbl_job_seekers.dated','tbl_job_seekers.dated','tbl_job_seekers.employer_id','tbl_team_member.full_name',
                        'tbl_team_member.team_member_type','tbl_team_member_type.type_ID',
                        'tbl_team_member_type.type_name','tbl_team_member.company_id','tbl_job_seekers.org_id'
                            )
        ...
        $job_details=Tbl_team_member_type::leftjoin('tbl_post_jobs','tbl_post_jobs.for_group','=','tbl_team_member_type.type_ID')
                    ...
                    'tbl_post_jobs.job_title as job_title','tbl_post_jobs.ID as Job_id',
                    'tbl_post_jobs.job_code as job_code',
        ...
        ->leftjoin('tbl_team_member_type','tbl_team_member_type.type_ID','=','tbl_team_member.team_member_type')  
        ->select('user.ID as ','tbl_job_seekers.ID','tbl_job_seekers.dated','tbl_seeker_applied_for_job.dated','tbl_job_seekers.employer_id',
    ...
    public function get_date($status="")
    {
        ...
        if($status==1)
        {
            ...
            for($i=0;$i<5;$i++)
            {
                $date_array['date_d'][]=date('Y-m-d', strtotime('-'.$i.' days'));
        ...
        //         $date_array['date_d'][]=date('Y-m-d', strtotime('-7*'.$i.' days'));
        //         // $date_array['date_d'][]=date($date_array['date_d'],strtotime('-1 days'));
        //     }
        // }
